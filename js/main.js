...
var ViewModel = function() {
	var self = this;
	self.map;										//Google map Map object
	self.latlong;									//Google map LatLng object
	 ...
	 * This function creates teh goodl represents the enemy bugs in the game
	...
	self.initializeMap = function() {
		latlong = new google.maps.LatLng(33.774671,-84.295911);
	 ...
	 * following types: store,bar,food,restaurant,cafe,florist,hair_care,spa,pharmacy.
	...
	self.createMarker = function(placeResult) {
	    var lat = placeResult.geometry.location.k;  // latitude from the place service
	    var lon = placeResult.geometry.location.D;  // longitude from the place service
	 ...
	 * This function will show or hide the map or list view
	 * based on the values of the showMap and showList observables.
	 */
	...
	self.setYellowPageInfo = function(location) {
	    ...
	    $.ajax({
	        dataType: "jsonp",
	        url: "http://api2.yp.com/search-api/search/devapi/search?searchloc=decatur+GA&term="+encodeURIComponent(location.name)+"&format=json&listingcount=1&key=c9fp548nz9",
	...
	//adds a listener to the selecteLocationType observable that will execute
	//the filterMarkers function anytime the selectedLocationType changes.
	self.selectedLocationType.subscribe( function(newValue) {
		self.filterMarkers();
	});
	...
	self.locations.subscribe( function(newValue) {
		self.filterMarkers();
	});
}
...
var getInfoWindowContent = function(placeResult) {
	...
	var content = "<div id="+placeResult.id+"><h4>"+placeResult.name+"</h4><p>"+placeResult.vicinity+",GA 30030 US</p><p>rating:  "+rating+"</p></div>";
	return content;
}
